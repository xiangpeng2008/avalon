...
python:{[id;command] 
  if[not " " in command;
    command,:"`";];
  `last_id set `$id; 
  //show "python[\"",id,"\";\"",command,"\"]"; 
  :get parse lower command;};
...
vote:{
  ...
  (1!records)[which_round;s_f_nb]:(1!records)[which_round;s_f_nb]+1;
  if[(get `votesNb)=missings[which_round];
    (1!records)[which_round;`status]:$[(1!records)[which_round;`failNb]>=missings[which_round];`fail;`sucess];
    which_round+:1;
    `votesNb set 0;];
  }
...
python[103112368316136110182;"im player xiangpeng"]
